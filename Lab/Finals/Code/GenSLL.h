...
// Return a ptr to the root of sublist1, var "root" is updated to point to head of sublist1
node *splitList(int start_index, int num)
{
    ...
    else if (start_index > nodeCount() - 1)
    {
        printf("Starting index must be less than total number of nodes (%d)!\n", nodeCount);
        return NULL;
    }
    if (num < 0)
    {
        printf("Length of sublist 1 must be a non-negative integer!\n");
        return NULL;
    }
    else if (num > (nodeCount() - start_index))
    {
        printf("Length of sublist 1 must be less than the remaining number of nodes !\n");
        return NULL;
    }
